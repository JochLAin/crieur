...
class Logger extends EventListener {
    ...
    error(message, options = {}) {
        ...
        if (message instanceof Error) {
            ...
            // Title
            console.log(`${chalk.red.bold(`${message.name}:`)} ${chalk.red(splitted.shift())}`);
            // Detail
            if (splitted.length) console.log(chalk.hex('#ff8a65')(`  ${splitted.join('\n  ')}`));
            // Stacktrace
    ...
    write(message, { background, color, bold, italic, strikethrough, underline, block } = {}, method = 'log') {
        ...
        if (this.is_browser) {
            ...
            if (color === 'blue') color = '#536dfe';
            else if (color === 'cyan') color = '#18ffff';
            else if (color === 'red') color = '#ff5252';
            else if (color === 'yellow') color = '#ffab40';
